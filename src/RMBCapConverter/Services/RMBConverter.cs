...
namespace RMBCapConverter.Services
{
    ...
    {
        public static Response<string> GetCapitalizedRmb(string input)
        {
            // Constants: 
            ...
            {
                return new Response<string>() { Message = "请输入小写金额！" };
            }

            if (double.Parse(input) > MAXIMUM_NUMBER)
            {
                return new Response<string>() { Message = "金额过大，应小于1000亿元！" };
            }
            ...
            return new Response<string>() { IsSuccess = true, Item = outputCharacters };
        }
    }
}
